...
export default function SendMessageListPresenterPage(props: any){
    ...
    <div>
    <S.SendWrapper>
        <S.Title>쪽지함 <S.RightOutline /> 보낸 쪽지함</S.Title>
        <S.SendHead>
            <S.MsgAmountDiv>총 <S.MsgAmountSpan>{props.count}</S.MsgAmountSpan>개의 받은 쪽지가 있습니다.</S.MsgAmountDiv>
            <S.WriteBtn onClick={props.onClickWriteMessage}><S.WriteImg src="/images/Send.png"/>쪽지 보내기</S.WriteBtn>
        </S.SendHead>
                ...
                {props.dataSendMessages?.fetchSendMessages.map((el:any)=>(
                    <>
                        <S.MessageBoxDiv key={uuidv4()}>
                                ...
                                <S.ProfileDiv>
                                    <S.ProfileImg src={profileUrl+el.messageReceivedUserImage}/>
                                    <S.SendUserDiv>{el.messageReceivedUser}</S.SendUserDiv>
                                </S.ProfileDiv>

                                <S.MessageContents onClick={props.onClickMessageDetail(el.messageInfo?.messageInfoId)}>{el.messageInfo?.messageInfoContents}</S.MessageContents>
